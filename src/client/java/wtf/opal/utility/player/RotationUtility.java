...
//        }
    ...
    public static float[] getRotationsTo(double targetX, double targetY, double targetZ, float currentYaw, float currentPitch) {
        return RotationUtility.getRotations(targetX, targetY, targetZ, currentYaw, currentPitch, 180.0f, 0.0f);
    }
    ...
    public static float[] getRotations(double targetX, double targetY, double targetZ, float currentYaw, float currentPitch, float maxAngle, float smoothFactor) {
        ...
        yawDelta = Math.abs(yawDelta) <= 1.0f ? 0.0f : RotationUtility.smoothAngle(RotationUtility.clampAngle(yawDelta, maxAngle), smoothFactor);
        pitchDelta = Math.abs(pitchDelta) <= 1.0f ? 0.0f : RotationUtility.smoothAngle(RotationUtility.clampAngle(pitchDelta, maxAngle), smoothFactor);
        return new float[]{RotationUtility.quantizeAngle(currentYaw + yawDelta), RotationUtility.quantizeAngle(currentPitch + pitchDelta)};
    }
